...
class App extends React.Component{
  ...
  componentMount(){
    fetch('https://interview-8e4c5-default-rtdb.firebaseio.com/front-end/202010013.json').then(res=>res.json())
    .then(response => {
  ...
  updateAPI = (counter) => {
    fetch('https://interview-8e4c5-default-rtdb.firebaseio.com/front-end.json',{
      method:"PUT",
      body:JSON.stringify({'202010013':counter})
    }).then(res=>res.json())
    .then(response=> {
      console.log(response)
  ...
  render() {
    ...
    return (
      <>
      <div>
      { !setDone && <div> <div class="lds-roller"><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div></div>Saving Counter Value</div>}
      </div>
